...
# Переключите раскладку на English (US), пожалуйста.
PC_KEYS_SEQUENCE = [
    # 1-я октава (C..C)
...
def env_held_vector(t: np.ndarray) -> np.ndarray:
    ...
    if ATTACK > 0:
        ...
        # мгновенный переход к 1 в момент начала
        m_a0 = (t >= 0) & (t < DECAY)
        if np.any(m_a0):
            env[m_a0] = 1.0
...
def env_vector_for_note(note: ActiveNote, frames_idx: np.ndarray) -> np.ndarray:
    # ADSR по вектору кадров для одной ноты
...
class PianoApp:
    def __init__(self):
        ...
        info = (
            ...
            "ПК-клавиатура: z s x d c v g b h n j m , q 2 w 3 e r 5 t 6 y 7 u. "
    ...
    def build_pc_keymap(self) -> Dict[str, int]:
        # Соответствие ПК-клавиш 25 клавишам диапазона
        mapping = {}
    ...
    def start_note(self, k: KeyInfo, velocity: float = 0.85, source_tag: Optional[str] = None, auto_release: bool = False) -> Optional[ActiveNote]:
        if k.freq is None:
            return None
        with self.notes_lock:
            # Voice stealing: если достигли лимита — отпустим самую тихую ноту
            if len(self.active_notes) >= MAX_POLYPHONY:
                # Оценим текущую амплитуду огибающей у всех
            ...
            # авто-релиз если нужно (для клика мышью, когда sustain не удерживается)
    ...
    def audio_callback(self, outdata, frames, time_info, status):
        ...
        buf = np.zeros(frames, dtype=np.float64)  # внутренние расчеты в float64
        ...
        with self.notes_lock:
            ...
            # Обработаем события авторелиза, которые попадают в текущий блок
            for note in notes:
            ...
            # Микширование нот (векторно по блоку)
            for ni, note in enumerate(notes):
                # Оценим, не закончился ли релиз к началу блока (для удаления после)
                if (note.release_frame is not None
                    and gf >= note.release_frame + int(RELEASE * sr) + 1):
                    to_remove_indices.add(ni)
                    continue

                # Огибающая для всего блока
                idx = gf + frames_idx
                env = env_vector_for_note(note, idx)
                if not np.any(env > 0.0):
                    # Если полностью ноль — пометим для удаления, если релиз завершился в этом блоке
                    if note.release_frame is not None and (gf + frames) >= note.release_frame + int(RELEASE * sr) + 1:
                        to_remove_indices.add(ni)
                    continue

                # Синтез гармоник (векторно)
                sig = np.zeros(frames, dtype=np.float64)
                for h_idx, (ratio, h_amp) in enumerate(HARMONICS):
                    phi0 = note.phases[h_idx]
                    inc = TWO_PI * (note.freq * ratio) / sr
                    ph = phi0 + inc * frames_idx
                    sig += np.sin(ph) * (h_amp * PARTIALS_AMPLITUDE)
                    # Обновить фазу
                    phi_end = phi0 + inc * frames
                    # нормализация в [0, 2π)
                    note.phases[h_idx] = phi_end % TWO_PI

                # Применяем огибающую и velocity
                sig *= env * note.velocity
                buf += sig

            # Удалим завершённые ноты
...
if __name__ == "__main__":
    ...
    print("Подсказка: переключите раскладку на English для игры с ПК-клавиатуры.")
    app.run()